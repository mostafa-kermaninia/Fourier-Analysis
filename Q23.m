...
% I define the function `fourier` which computes and plots the Fourier series approximation
% of a given function up to a specified number of terms.
function fourier(Num, P, a, Nshow) % I explain arguments in report
    ...
    figure;
    fplot(@(x) f(x), [-L L], 'LineWidth', 3.5);
    hold on;
    plot(x, F_s, 'r', 'LineWidth', 1.5);
    legend('Actual Function', 'Fourier Series');
    xlabel('x');
    ylabel('Vertical Axis');
    title(['Fourier Series / x^', num2str(a), ' / ', num2str(Nshow), ' terms used in plot']);
    grid on;
    hold off;
